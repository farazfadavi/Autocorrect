...
class Edit(object):
    ...
    def __init__(self, editedWord, corruptLetters, correctLetters):
        ...
        # Represents x in the "P(x|w)" error probability term of the noisy
        # channel model
        self.corruptLetters = corruptLetters
        # Represents w in the "P(x|w)" error probability term of the noisy
        # channel model
...
class EditModel(object):
    ...
    def deleteEdits(self, word):
        ...
        for i in xrange(1, len(word)):
            # The corrupted signal are this character and the character
            # preceding
            corruptLetters = word[i - 1:i + 1]
        #   print colored("\tcorruptLetters: "  + corruptLetters, 'green')
            # The correct signal is just the preceding character
            correctLetters = corruptLetters[:-1]
        #   print colored("\tcorrectLetters: "  + correctLetters, 'blue')

            # The corrected word deletes character i (and lacks the start
            # symbol)
            correction = "%s%s" % (word[1:i], word[i + 1:])
            # print colored("\tcorrection: " + correction + " corruptLetters: " + corruptLetters + " correctLetters: " + correctLetters, 'yellow')
    ...
    def insertEdits(self, word):
        ...
        for i in xrange(1, len(word) + 1):
            ...
            correction = 'a'
            for elem in EditModel.ALPHABET:
                correction = "%s%s%s" % (word[1:i], elem, word[i:])
                # print colored("\tcorrection: " + correction + " corruptLetters: " + corruptLetters + " correctLetters: " + corruptLetters[0] + elem, 'yellow')
                ret.append(Edit(correction, corruptLetters, corruptLetters[0] + elem))
        return ret
    ...
    def transposeEdits(self, word):
        ...
        for i in xrange(0, len(word) - 1):
            corruptSignal = word[i: i + 2]
            correctSignal = corruptSignal[1] + corruptSignal[0]
            correction = word[:i] + correctSignal + word[i + 2:]
            # print colored("\tcorrection: " + correction + " corruptSignal: " + corruptSignal + " correctSignal: " + correctSignal, 'yellow')
            if correction != word:
                ret.append(Edit(correction, corruptSignal, correctSignal))
        return ret
    ...
    def replaceEdits(self, word):
        ...
        for i in xrange(0, len(word)):
            ...
            correction = 'a'
            for elem in EditModel.ALPHABET:
                if elem != corruptLetters:
                    correction = "%s%s%s" % (word[:i], elem, word[i + 1:])
                    # print colored("\tcorrection: " + correction + " corruptLetters: " + corruptLetters + " correctLetters: " + elem, 'yellow')
                    ret.append(Edit(correction, corruptLetters, elem))
        return ret

    def edits(self, word):
        """Returns a list of tuples of 1-edit distance words and rules used to generate them, e.g. ("test", "te|et")"""
        # Note: this is just a suggested implementation, feel free to modify it
        # for efficiency
...
def main():
    ...
    DELETE_EDITS = set(['Edit(editedWord=i, rule=<h|<)',
                        'Edit(editedWord=h, rule=hi|h)'])
    INSERT_EDITS = set([Edit('ahi', '<', '<a'), Edit('bhi', '<', '<b'), Edit('chi', '<', '<c'), Edit('dhi', '<', '<d'), Edit('ehi', '<', '<e'), Edit('fhi', '<', '<f'), Edit('ghi', '<', '<g'), Edit('hhi', '<', '<h'), Edit('ihi', '<', '<i'), Edit('jhi', '<', '<j'), Edit('khi', '<', '<k'), Edit('lhi', '<', '<l'), Edit('mhi', '<', '<m'), Edit('nhi', '<', '<n'), Edit('ohi', '<', '<o'), Edit('phi', '<', '<p'), Edit('qhi', '<', '<q'),
                        Edit('rhi', '<', '<r'), Edit('shi', '<', '<s'), Edit('thi', '<', '<t'), Edit('uhi', '<', '<u'), Edit('vhi', '<', '<v'), Edit('whi', '<', '<w'), Edit('xhi', '<', '<x'), Edit('yhi', '<', '<y'), Edit(
                            'zhi', '<', '<z'), Edit('hai', 'h', 'ha'), Edit('hbi', 'h', 'hb'), Edit('hci', 'h', 'hc'), Edit('hdi', 'h', 'hd'), Edit('hei', 'h', 'he'), Edit('hfi', 'h', 'hf'), Edit('hgi', 'h', 'hg'), Edit('hhi', 'h', 'hh'),
                        Edit('hii', 'h', 'hi'), Edit('hji', 'h', 'hj'), Edit('hki', 'h', 'hk'), Edit('hli', 'h', 'hl'), Edit('hmi', 'h', 'hm'), Edit('hni', 'h', 'hn'), Edit('hoi', 'h', 'ho'), Edit('hpi', 'h', 'hp'), Edit('hqi', 'h', 'hq'), Edit(
                            'hri', 'h', 'hr'), Edit('hsi', 'h', 'hs'), Edit('hti', 'h', 'ht'), Edit('hui', 'h', 'hu'), Edit('hvi', 'h', 'hv'), Edit('hwi', 'h', 'hw'), Edit('hxi', 'h', 'hx'), Edit('hyi', 'h', 'hy'), Edit('hzi', 'h', 'hz'),
                        Edit('hia', 'i', 'ia'), Edit('hib', 'i', 'ib'), Edit('hic', 'i', 'ic'), Edit('hid', 'i', 'id'), Edit('hie', 'i', 'ie'), Edit('hif', 'i', 'if'), Edit('hig', 'i', 'ig'), Edit('hih', 'i', 'ih'), Edit('hii', 'i', 'ii'), Edit(
                            'hij', 'i', 'ij'), Edit('hik', 'i', 'ik'), Edit('hil', 'i', 'il'), Edit('him', 'i', 'im'), Edit('hin', 'i', 'in'), Edit('hio', 'i', 'io'), Edit('hip', 'i', 'ip'), Edit('hiq', 'i', 'iq'), Edit('hir', 'i', 'ir'),
    ...
    REPLACE_EDITS = set([Edit('ai', 'h', 'a'), Edit('bi', 'h', 'b'), Edit('ci', 'h', 'c'), Edit('di', 'h', 'd'), Edit('ei', 'h', 'e'), Edit('fi', 'h', 'f'), Edit('gi', 'h', 'g'), Edit('ii', 'h', 'i'), Edit('ji', 'h', 'j'),
                         Edit('ki', 'h', 'k'), Edit('li', 'h', 'l'), Edit('mi', 'h', 'm'), Edit('ni', 'h', 'n'), Edit('oi', 'h', 'o'), Edit(
                             'pi', 'h', 'p'), Edit('qi', 'h', 'q'), Edit('ri', 'h', 'r'), Edit('si', 'h', 's'), Edit('ti', 'h', 't'),
                         Edit('ui', 'h', 'u'), Edit('vi', 'h', 'v'), Edit('wi', 'h', 'w'), Edit('xi', 'h', 'x'), Edit('yi', 'h', 'y'), Edit('zi', 'h', 'z'), Edit('ha', 'i', 'a'), Edit(
                             'hb', 'i', 'b'), Edit('hc', 'i', 'c'), Edit('hd', 'i', 'd'), Edit('he', 'i', 'e'), Edit('hf', 'i', 'f'), Edit('hg', 'i', 'g'), Edit('hh', 'i', 'h'), Edit('hj', 'i', 'j'),
                         Edit('hk', 'i', 'k'), Edit('hl', 'i', 'l'), Edit('hm', 'i', 'm'), Edit('hn', 'i', 'n'), Edit('ho', 'i', 'o'), Edit(
        'hp', 'i', 'p'), Edit('hq', 'i', 'q'), Edit('hr', 'i', 'r'), Edit('hs', 'i', 's'), Edit('ht', 'i', 't'),
